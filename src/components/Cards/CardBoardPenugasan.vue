<template>
  <div class="w-full lg:w-3/12 px-4 mt-14">
    <div class="relative w-full mb-3 bg-white p-4 shadow-lg rounded">
      ...
      <draggable :class="`list-group kanban-column ${status}`" @end="(event) => $emit('onDragEnd', event, event.draggedItem, status)" :list="items" group="stories">
        <div class="list-group-item rounded bg-blue-500 text-white p-2 mb-4 cursor-pointer" v-for="element in items" :key="element.id_story" @click="handleShowDetail(element)">

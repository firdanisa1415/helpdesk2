<template>
  <div class="w-full lg:w-3/12 px-4 mt-14">
    <div class="relative w-full mb-3 bg-white p-4 shadow-lg rounded">
      ...
      <draggable :class="`list-group kanban-column ${status}`" @end="(event) => $emit('onDragEnd', event, event.draggedItem, status)" :list="items" group="report">
        <div class="list-group-item rounded bg-blue-500 text-white p-2 mb-4 cursor-pointer" v-for="element in items" :key="element.id" @click="handleShowDetail(element)">{{ element.id_pelaporan }} - {{ element.judul_pelaporan }}</div>
      </draggable>
    </div>
        ...
        <modal :show="detailPelaporan">
          ...
          <template #body>
            ...
            <div class="flex flex-wrap">
              <div class="lg:w-3/12">
                <div class="relative w-full mb-3">
                  <h2 class="block uppercase text-blueGray-600 text-xs font-bold mb-2">Tanggal Laporan</h2>
                </div>
              </div>
              <div class="lg:w-9/12">
                <div class="relative w-full mb-3">
